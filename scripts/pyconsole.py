...
c_shiftmap 	= {'`':'~',';':':',',':'<','.':'>','/':'?','\'':'\"', '-': '_',
			   ...
			   '7': '&', '6': '^', '9': '(', '8': '*', '=': '+', '[': '{',
			   ']': '}', '\\': '|', 'a': 'A', 'c': 'C', 'b': 'B', 'e': 'E',
			   'd': 'D', 'g': 'G', 'f': 'F', 'i': 'I', 'h': 'H', 'k': 'K',
			   'j': 'J', 'm': 'M', 'l': 'L', 'o': 'O', 'n': 'N', 'q': 'Q',
			   'p': 'P', 's': 'S', 'r': 'R', 'u': 'U', 't': 'T', 'w': 'W',
			   'v': 'V', 'y': 'Y', 'x': 'X', 'z': 'Z'}
...
class Writable(list):
	...
	def readline(self, size=-1):
		# Python's interactive help likes to try and call this.
		# Don't count on it ever being implemented.
...
class Console:
	...
	def getvar(self, name):
		"""Gets the value of a variable, useful for people that want to
		...
		for event in eventlist:
			if event.type == KEYDOWN:
				...
				if event.key in range(32,127):
					...
					char = chr(event.key)
					if mods & KMOD_SHIFT and char in c_shiftmap:
						char = c_shiftmap[char]
					if mods & KMOD_CTRL:
						if char in self.key_calls:
							self.key_calls[char]()
					else:
	...
	def convert_token(self, tok):
		...
		try:
			tmp = eval(tok, self.__dict__, self.user_vars)
		...
		while i < len(tokens):
			...
			if re_is_number.match(val):
				cmd.append(self.convert_token(val))
		 		...
		 		cmd.append(self.convert_token(val.strip("$")))
			elif val == "True":
				cmd.append(True)
			elif val == "False":
				cmd.append(False)
	...
	def load_cfg(self):
		...
		cfg_path = os.path.join(path,"../data/pyconsole.cfg")
